...
curl -s -o NUL --retry 5 --retry-delay 10 -H "apikey: %SB_ANON%" -H "Authorization: Bearer %SB_ANON%" "%SB_URL%/rest/v1/forum_topics?select=id&limit=1"
echo [%date% %time%] boot ping rest exit=%errorlevel% >> "%~dp0keepalive.log"

rem ---- Auth ping ----
curl -s -o NUL --retry 3 --retry-delay 10 "%SB_URL%/auth/v1/health"
